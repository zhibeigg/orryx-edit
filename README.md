...
当前版本：`0.16.29`，数据库 Schema：`12`，Editor 协议：V1/V2。
...
- Registry 提供 `options` 或类型级 `enumValues` 时，输入槽使用可搜索选择器；实体、药水、声音、材质等大目录仅渲染匹配窗口，并保留目录外自定义值与 Raw Kether 片段模式。
- Scratch 将普通字段卡片与嵌套输入子轨道分离：每张卡片保持自身自然高度，子树按最小可读宽度向右扩展；超出工作区的部分由画布统一提供横向滚动。
