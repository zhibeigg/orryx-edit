...
```bash
sudo install -o orryx -g orryx -m 0640 orryx-editor-0.10.13.jar /opt/orryx-editor/orryx-editor.jar
...
RELEASE_DATA_BASE_URL=https://editor.example.com
RELEASE_SIGNING_KEY_ID=release-key-2025-01
RELEASE_SIGNING_PRIVATE_KEY=...
RELEASE_SIGNING_PUBLIC_KEY=...
```

私钥使用 PKCS#8 DER 的 Base64，公钥使用 X.509 DER 的 Base64。插件侧必须预置相同 `keyId` 的可信公钥。密钥不完整、URL 非 HTTPS、插件未声明 `release.control.v1` 或仍连接 V1 时，服务端 fail-closed 拒绝发布。
