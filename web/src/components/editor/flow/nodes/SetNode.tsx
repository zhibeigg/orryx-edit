  ...
  return (
    <div className="relative overflow-visible" style={{ width: nodeData.layout?.width ?? 360 }}>
      ...
      <div className={`rounded-md border bg-[#111318] transition-shadow duration-150 ${selected ? "border-amber-300 shadow-[0_0_0_2px_rgba(251,191,36,0.22),0_12px_24px_rgba(0,0,0,0.3)]" : "border-amber-800/80 shadow-[0_8px_18px_rgba(0,0,0,0.22)]"}`}>
        <div className="flex min-h-10 items-center justify-between gap-4 rounded-t-[5px] bg-amber-950 px-4 py-2 text-[14px] font-semibold text-amber-50">
          <span>设置变量</span>
          <span className="font-mono text-[11px] uppercase tracking-[0.1em] text-amber-200/60">set</span>
        </div>

        <div className="grid gap-3 px-4 py-3">
          <label className="grid min-h-10 grid-cols-[92px_minmax(0,1fr)] items-center gap-4 text-[13px]">
            ...
            <input
              ...
              className={`${NODE_CONTROL_CLASS} min-h-9 w-full min-w-0 rounded border border-white/10 bg-black/35 px-3 py-2 text-[13px] text-white focus:outline-none focus:ring-1 focus:ring-amber-300/70 disabled:cursor-not-allowed disabled:opacity-55`}
            />
          </label>

          <label className="relative grid min-h-11 grid-cols-[92px_minmax(0,1fr)] items-center gap-4 text-[13px]">
            <Handle
              type="target"
