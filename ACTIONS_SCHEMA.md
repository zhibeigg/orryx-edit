...
Orryx Editor 0.15.24 不再把构建时 JSON 当作唯一数据源。服务端只读取 Orryx 官方 GitHub Pages 的 `stable` 通道：
...
Orryx Editor 0.15.24 的 Flow Editor 直接使用唯一的 `input.key` 作为左侧数据输入 Handle ID，并使用固定的 `output`、`flow-in`、`flow-out` 标识数据输出和执行端口。节点真实尺寸、容器 slot 尺寸和自动布局坐标属于编辑器瞬态数据，不进入 Actions Schema，也不改变 Kether 文本语义。
