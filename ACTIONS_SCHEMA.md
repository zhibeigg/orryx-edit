...
Orryx Editor 0.16.30 不再把构建时 JSON 当作唯一数据源。服务端只读取 Orryx 官方 GitHub Pages 的 `stable` 通道：
...
Orryx Editor 0.16.30 的 Action 解析游标与 TabooLib 6.3.0 `SimpleReader` / `KetherScriptLoader.Reader` 对齐：普通 token 只由空白分隔，`[`、`]`、`{`、`}` 由当前列表或匿名块解析器拥有并递归消费，因此 `&event[key]`、`player[name]` 不会被误识别成外层列表边界。`expect` 对必需的 `then`、`in` 和闭合符执行严格校验；失败时回滚并把完整、括号平衡的局部 Action 保存为 Raw，而不是留下孤立的 `]`、`}` 或 `then` 块。
...
Orryx Editor 0.16.30 的 Flow Editor 直接使用唯一的 `input.key` 作为左侧数据输入 Handle ID，并使用固定的 `output`、`flow-in`、`flow-out` 标识数据输出和执行端口。Scratch 中普通字段保留固定的最小可读列宽，嵌套输入则作为父卡片右侧的独立子轨道渲染；子树宽度逐层向右增长，但单张卡片高度不继承后代高度，超出工作区后只由画布横向滚动。该布局不允许父输入槽压窄或裁切子块。节点真实尺寸、容器 slot 尺寸和自动布局坐标属于编辑器瞬态数据，不进入 Actions Schema，也不改变 Kether 文本语义。
