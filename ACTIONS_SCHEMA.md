...
Orryx Editor 0.15.25 不再把构建时 JSON 当作唯一数据源。服务端只读取 Orryx 官方 GitHub Pages 的 `stable` 通道：
...
Orryx Editor 0.15.25 的 Action 解析游标与 TabooLib 6.3.0 `SimpleReader` / `KetherScriptLoader.Reader` 对齐：普通 token 只由空白分隔，`[`、`]`、`{`、`}` 由当前列表或匿名块解析器拥有并递归消费，因此 `&event[key]`、`player[name]` 不会被误识别成外层列表边界。`expect` 对必需的 `then`、`in` 和闭合符执行严格校验；失败时回滚并把完整、括号平衡的局部 Action 保存为 Raw，而不是留下孤立的 `]`、`}` 或 `then` 块。

`grammar.sequence` 可声明 `input`、`literal`、`localRaw`、`branch`、`optional` 与 `actionList`。`actionList` 对应 TabooLib 的 `ArgTypes.listOf(ArgTypes.ACTION)`，用于 `all`、`any`、`seq`、`await_all`、`await_any` 和 `array`。未声明完整 grammar 的 Action 继续使用 `localRawRemainder`，且 grammar 参数无法完整映射到公开 inputs 时必须整段 Raw 保真，禁止在保存时静默丢参。

Orryx Editor 0.15.25 的 Flow Editor 直接使用唯一的 `input.key` 作为左侧数据输入 Handle ID，并使用固定的 `output`、`flow-in`、`flow-out` 标识数据输出和执行端口。节点真实尺寸、容器 slot 尺寸和自动布局坐标属于编辑器瞬态数据，不进入 Actions Schema，也不改变 Kether 文本语义。
